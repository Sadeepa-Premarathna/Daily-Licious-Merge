...
// Future: Import Payroll model when available
// import Payroll from '../finance_Model/PayrollModel.js';

// Configuration for data source (mock vs database)
const USE_MOCK_DATA = true; // Set to false when Payroll collection is available
...
const getAllPayrollRecords = async () => {
  if (USE_MOCK_DATA) {
    ...
    // Use real database when Payroll collection is available
    // Uncomment when Payroll model is imported:
    // try {
    //   const payrollRecords = await Payroll.find({ status: 'Active' });
    //   return payrollRecords;
    // } catch (error) {
    ...
    console.warn('Payroll database not available, falling back to mock data');
    return getMockPayrollData();
  }
};
